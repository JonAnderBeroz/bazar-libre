  ...
  return (
    <form onSubmit={handleFormSubmit} className="flex flex-col gap-4 items-center">
      <article className="bg-[#ebebeb] text-black py-1 flex">
        ...
        />
        <button type="submit" className="pr-2" aria-label="Search Button with icon">
          <Search />
        </button>
      </article>
      {showButton && (
        <button
          type="submit"
          className="rounded-full bg-gray-400 w-32 text-black"
          aria-label="Search Button with no icon"
        >
          Buscar
        </button>
      )}
